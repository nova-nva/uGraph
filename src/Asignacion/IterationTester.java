...
public class IterationTester {
    ...
    public IterationTester(int[][] matrix) {
        this.matrix = matrix;
    ...
    public Vector<Vector<Vector<Integer>>> simpleSearch(int[][] data){
    ...
    public static void main(String[] args){
        ...
        AsignationSolver solver = new AsignationSolver(finalTest3);
        System.out.println(solver.solve());

        /*

        IterationTester tester = new IterationTester(finalTest2);

        // initialize matrixes by cloning original matrix
        int dimension = tester.getDimension();
        int[][] posibility = new int[dimension][dimension];
        for(int i = 0; i<dimension; i++){
            for(int j = 0; j<dimension; j++){
                posibility[i][j] = finalTest2[i][j];
            }
        }
        ...
        solutions = 0;
        posibility = tester.maximize(posibility);
        ...
        while(solutions < dimension){
            // iteration process
            Vector<Vector<Vector<Integer>>> singlePosibilitiesIndex = tester.simpleSearch(posibility);
