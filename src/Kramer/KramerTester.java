...
*/
    ...
    public static int[][] cloneMatrix(int[][] original, int rows, int cols){
    ...
    public static int[][] iterate(int[][] lastAnswer, int[][] lastSolution, Vector<Vector<Integer>> solutionIndex,int rows, int cols){
        // TODO: get new solution - parse X
        // encontrar numero por encima de la solucion
        int x = 0;
        Vector<Integer> xIndex = new Vector<>();

        // hallar el menor para que sea el x
                ...
                if(lastAnswer[i][j] > 0){
                    if(lastAnswer[i][j] > xMayor){
                        ...
                        for(int a = 0; a<solutionIndex.size(); a++){
                            if(j == solutionIndex.get(a).get(1)){
                                giantCounter++;
                            }
                        }
                        System.out.println("Giant Counter " + i + "-" + j + " -> " + giantCounter);
                        if(giantCounter <= 2) {
        ...
        boolean zeroConsistency = false, poligonalConsistency = true;
        while(!end){
    ...
    public static int[][] powerIterate(int[][] lastAnswer, int[][] originalLastSolution, Vector<Vector<Integer>> originalSolutionIndex, int rows, int cols, boolean minimize){
        // TODO: get new solution - parse X
        // encontrar numero por encima de la solucion
        int x = 0;
        Vector<Integer> xIndex = new Vector<>();
        ...
        Vector<Vector<Integer>> solutionIndex;
        int[][] lastSolution;
        do{
    ...
    public static int getTotalAttribute(int[][] solution, int[][] generalData, Vector<Vector<Integer>> indexes){
        int total = 0;
    ...
    public static boolean isIterable(int[][] matrix, int rows, int cols, boolean minimize){
        if(minimize) {
            for (int i = 0; i < rows; i++) {
                for (int j = 0; j < cols; j++) {
                    if (matrix[i][j] > 0)
    ...
    public static int[][] getSolutionMatrix(Vector<Integer> verticalGenerator, Vector<Integer> horizontalGenerator){
    ...
    public static int[][] matrixDiff (int[][] first, int[][] second, int rows, int cols){
        // restar first - second
    ...
    public static Vector<Vector<Integer>> getGenerators(int[][] data, Vector<Vector<Integer>> solutionIndex, int rows, int cols){
    ...
    public static Vector<Vector<Integer>> fixGenerators(Vector<Vector<Integer>> generators, Vector<Integer> currentSolution, Vector<Vector<Integer>> solutionIndex){
        boolean direction = true; // true -> horizontal; false -> vertical
    ...
    public static Vector<Vector<Integer>> fixGenerators2(Vector<Vector<Integer>> generators, Vector<Integer> currentSolution, Vector<Vector<Integer>> solutionIndex, boolean direction){
        // clonamos los datos para mantener los iniciales intactos
        Vector<Vector<Integer>> generatorsClone = new Vector<>(generators);
        Vector<Integer> solutionClone = new Vector<>(currentSolution);
        ...
        for(int i = 0; i<currentSolution.size(); i++) {
            if (currentSolution.get(i) != 0) {
                // tomamos los sub indices
                x = solutionIndex.get(i).get(0);
                y = solutionIndex.get(i).get(1);

                // tomamos el dato a operar
                dato = currentSolution.get(i);
            }
        }
    ...
    public static boolean validateGenerators(Vector<Integer> solution){
    ...
    public static void print(boolean[][] data, int rows, int cols){
        for(int i = 0; i<rows; i++) {
            for (int j = 0; j < cols; j++) {
                System.out.print(data[i][j] + "\t");
            }
